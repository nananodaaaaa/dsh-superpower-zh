...
description: >-
  ...
  把内容做成 PPT 图片"时使用。本技能**只产出图片型 PPT**（每页 .png + 合成的 .pptx）；若还要可编辑 pptx，接着用 gorden-image2pptx。
---

# gorden-image-ppt-gen — 生成图片格式的 PPT
...
4. **少用绿色**：尽量避免大面积纯绿（下游 gorden-image2pptx 默认绿幕抠图会冲突）。
...
- 下一步要可编辑 pptx → 用 **gorden-image2pptx**；要一键全流程 → 用 **gorden-super-ppt-skill**。
