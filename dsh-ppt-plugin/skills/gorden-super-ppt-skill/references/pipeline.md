...
## 阶段 1 · 生成图片型 PPT（gorden-image-ppt-gen）
...
> 详见 `../gorden-image-ppt-gen/references/image-prompt-guide.md`（§0 内容优先、§1.5 复杂度、§1.7 唯一框架+清晰优先、§4 图表目录、§7 outline 结构）。

## 阶段 2 · 还原为可编辑 PPTX（gorden-image2pptx）
...
> 详见 `../gorden-image2pptx/references/image-to-pptx.md`（B1–B9 全流程 + schema + 抠图保真铁律）。
